{ ***************************************************** }
{ TBrowseForFolder component version 1.0 (Nov. 5, 2002) }
...
type
  ...
  {
   The TBrowseForFolder component encapsulates the SHBrowseForFolder
   Shell API function
  }
...
procedure Register;
begin
  RegisterComponents('Dim', [TBrowseForFolder]);
end;

{ TBrowseForFolder }

function BrowseForFolderCallBack(Wnd: HWND; Msg: UINT;
  lParam: PItemIDList; lpData: TBrowseForFolder): Integer stdcall;
begin
 case Msg of
...
procedure TBrowseForFolder.CallbackInitialized(Wnd: HWND);
var
 Rect: TRect;
...
procedure TBrowseForFolder.CallbackSelChanged(Wnd: HWND;
  PIDL: PItemIDList);
var
...
procedure TBrowseForFolder.DoSelChanged(Path: string; var EnableOk: Boolean);
begin
 if Assigned(FOnSelChanged) then FOnSelChanged(Self, Path, EnableOk);
 SelChanged(Path, EnableOk);
end;

procedure TBrowseForFolder.DoSetPosition(var Left, Top: Integer; Width,
  Height: Integer);
begin
...
procedure TBrowseForFolder.SelChanged(Path: string; var EnableOk: Boolean);
begin
 // no action by default
end;

procedure TBrowseForFolder.SetOnSelChanged(const Value: TOnSelChanged);
begin
  FOnSelChanged := Value;
end;

procedure TBrowseForFolder.SetOnSetPosition(const Value: TOnSetPosition);
begin
  FOnSetPosition := Value;
end;

procedure TBrowseForFolder.SetPosition(var Left, Top: Integer; Width,
  Height: Integer);
begin
 // no action by default
end;

procedure TBrowseForFolder.SetStatusText(const StatusText: string);
begin
